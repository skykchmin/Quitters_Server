module.exports = function(app){
    ...
    cron.schedule('1 0 0-23/4 1-31 * *', certificationscheduler.updateChallengeParticipation, {
        timezone: "Asia/Seoul"
    }); // 챌린지 인증 테이블 추가

    cron.schedule('1 0 0-23/4 1-31 * *', certificationscheduler.updateIntermediateCertification, {
        timezone: "Asia/Seoul"
    }); // 챌린지 중간 인증 추가

    cron.schedule('1 0-59/1 * 1-31 * *', certificationscheduler.patchAutoChallengeIntermediateCertification, {
        timezone: "Asia/Seoul"
    }); // 챌린지 중간 인증 자동 전환

    cron.schedule('1 0 0 1-31 * *', certificationscheduler.updateChallengeSuccess, {
        timezone: "Asia/Seoul"
    }); // 챌린지 종료 날짜에 따른 챌린지 자동 전환 - 하루마다 
